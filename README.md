...
Two eval paths exist — one per retriever config:

| Script | Retriever | What it measures |
|--------|-----------|-----------------|
| `scripts/eval_tfidf.py` | TF-IDF (demo / Streamlit Cloud) | Hit@k on the deployed config |
| `python -m scripts.run_pipeline eval` | Hashing embedder + Qdrant | Hit@k on the full-pipeline config |

**Current results — TF-IDF retriever (`eval_results/sweep_results.json`):**

| top_k | Hit Rate | Latency (20 q) |
|-------|----------|----------------|
| 1 | 100% | ~500 ms |
| 3 | 100% | ~470 ms |
| 5 | 100% | ~480 ms |
...
> **Caveat:** The eval set was auto-generated by `scripts/make_eval_set.py` — each question contains the first 80 characters of its target chunk. This is a **self-recall** benchmark (TF-IDF finds text it was built from), not a held-out quality evaluation. The 100% figure reflects that; it is not a claim about semantic search quality. A proper eval would require human-written questions not derived from the corpus text.
...
# 6. Run the deterministic retrieval eval (hashing embedder + Qdrant)
python -m scripts.run_pipeline eval

# 6b. Run the eval against the deployed TF-IDF retriever (demo config)
